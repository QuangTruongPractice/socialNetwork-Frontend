...
import { createInvitation } from "../configs/LoadData";
    ...
    const fetchResults = async () => {
      ...
      try {
        const [accRes, groupRes] = await Promise.all([
          Apis.get(endpoints["accounts"], { params: { email: search } }),
          Apis.get(endpoints["groups"], { params: { name: search } }),
        ]);
        ...
        const mappedGroup = groupRes.data.map((g) => ({
          type: "group",
          id: g.id,
          name: g.name,
        }));
